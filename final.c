  ...
  if (write_symbols == GDB_DEBUG)
    {
      ...
      fprintf (file, "\t.gdbbeg %d\015", next_block_index++);
    }
     ...
     int optimize;
{
  ...
  if (write_symbols == GDB_DEBUG)
    fprintf (file, "\t.gdbend %d\015", pending_blocks[0]);
     ...
     int nopeepholes;
{
  ...
  switch (GET_CODE (insn))
      ...
      else if (NOTE_LINE_NUMBER (insn) == NOTE_INSN_BLOCK_END)
	{
	  ...
	  if (write_symbols == GDB_DEBUG)
	    fprintf (file, "\t.gdbend %d\015", pending_blocks[block_depth]);
	}
...
#else
	      fprintf (file, "\t.stabs \"%s\",%d,0,0,Ltext\015",
		       filename, N_SOL);
#endif
	      lastfile = filename;
	    }
     ...
     rtx *operands;
{
  ...
  putc (TARGET_NEWLINE, asm_out_file);
}

