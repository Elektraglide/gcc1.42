...
fatal_io_error (name)
     char *name;
{
  fprintf (stderr, "%s: %s: I/O error\015", progname, name);
  exit (35);
}
     ...
     char *file;
{
  ...
  if (last_error_function != current_function_decl)
	...
	fprintf (stderr, "In method %s:\015",
     ...
     char *name;
{
  ...
  fprintf (asm_out_file, "gcc_compiled\015");
#else
...
/*   fixup_stack_slots (); */
  ...
  if (optimize && flag_delayed_branch)
      ...
      if (dbr_sched_dump)
	{
	  TIMEVAR (dump_time,
		 {
		   fprintf (dbr_sched_dump_file, "\015;; Function %s\015\015",
			    IDENTIFIER_POINTER (DECL_NAME (decl)));
...
#endif
	    fprintf (stderr, " compiled by GNU C version %s.\015", __VERSION__);
#else
	    fprintf (stderr, " compiled by CC.\015");
#endif
