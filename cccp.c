...
#define CC_INCLUDE_DIR "/lib/include"
#endif
     ...
     char **argv;
{
  ...
  if (fp->length > 0 && fp->buf[fp->length-1] != TARGET_NEWLINE)
    fp->buf[fp->length++] = TARGET_NEWLINE;
  fp->buf[fp->length] = '\0';
  ...
  if (print_deps) {
    ...
    putc (TARGET_NEWLINE, deps_stream);
  ...
  while (1) {
    ...
    *obp++ = c;
	...
	while (1) {
	  if (is_hor_space[*bp])
	    bp++;
	  else if (*bp == '\\' && bp[1] == TARGET_NEWLINE)
	    bp += 2;
	...
	*obp++ = *ibp;
	...
	case '\\':
	  ...
	  if (*ibp == TARGET_NEWLINE) {
	    ...
	    while (*ibp == '\\' && ibp[1] == TARGET_NEWLINE) {
	      ibp += 2;
	      ++ip->lineno;
	    }
	    *obp++ = *ibp++;
	...
	*obp++ = '*';
    ...
    case '5': case '6': case '7': case '8': case '9':
	 ...
	 as an identifier.  Periods also, for sake of "3.e7".  */
      ...
      if (ident_length == 0) {
	while (ibp < limit) {
	  while (ibp < limit && ibp[0] == '\\' && ibp[1] == TARGET_NEWLINE) {
	    ++ip->lineno;
	    ibp += 2;
	  }
	  c = *ibp++;
     ...
     FILE_BUF *ip, *op;
{
  ...
  if (ident_length == 0 && *after_ident == TARGET_NEWLINE) {
    ip->bufp = after_ident;
    return 1;
  }
     ...
     register U_CHAR *p;
{
  ...
  while (1) {
    if (*p == '\\' && p[1] == TARGET_NEWLINE)
      p += 2;
    if (is_hor_space[*p])
      p++;
    else if (*p == '/') {
      if (p[1] == '\\' && p[2] == TARGET_NEWLINE)
	newline_fix (p + 1);
      if (p[1] == '*') {
	p += 2;
     ...
     int *line_counter;		/* place to remember newlines, or NULL */
{
  ...
  if (cplusplus && bp[-1] == '/') {
    if (output) {
      while (bp < limit)
	if ((*op->bufp++ = *bp++) == TARGET_NEWLINE) {
	  bp--;
	  break;
	}
      op->bufp[-1] = '*';
      *op->bufp++ = '/';
      *op->bufp++ = TARGET_NEWLINE;
    } else {
      while (bp < limit) {
	if (*bp++ == TARGET_NEWLINE) {
	  bp--;
  ...
  while (1) {
	...
	*eofp = 1;
	...
	bp += 2;
      ...
      if (*bp == TARGET_NEWLINE && count_newlines) {
	if (backslash_newlines_p)
	  *backslash_newlines_p = 1;
	++*count_newlines;
      }
     ...
     enum file_change_code file_change;
{
  ...
  line_cmd_buf[len++] = TARGET_NEWLINE;
  check_expand (op, len + 1);
  if (op->bufp > op->buf && op->bufp[-1] != TARGET_NEWLINE)
    *op->bufp++ = TARGET_NEWLINE;
  ...
  if (nargs >= 0) {
	...
	if (ap->stringify != 0) {
	  ...
	  for (; i < arglen; i++) {
	    ...
	    if (c == TARGET_NEWLINE && arg->raw[i+1] != TARGET_NEWLINE) {
	      i++;
	      continue;
	    }
	    ...
	    if (! in_string
		&& (c == TARGET_NEWLINE ? arg->raw[i+1] == TARGET_NEWLINE : is_space[c])) {
	      while (1) {
		/* Note that Newline Space does occur within whitespace
		   sequences; consider it part of the sequence.  */
		if (c == TARGET_NEWLINE && is_space[arg->raw[i+1]])
		  i += 2;
		else if (c != TARGET_NEWLINE && is_space[c])
		  i++;
		else break;
		c = arg->raw[i];
	      }
	  ...
	  if (ap->raw_after) {
		...
		while (p2 != p1 && p2[-1] == TARGET_NEWLINE) p2--;
		if ((l1 - 1 - p2) & 1) {
		  l1 -= 2;
		}
		else break;
     ...
     int *depthptr, *newlines, *comments;
{
  ...
  while (bp < limit) {
    ...
    case TARGET_NEWLINE:
      ...
      break;
    case '/':
      if (bp[1] == '\\' && bp[2] == TARGET_NEWLINE)
	newline_fix (bp + 1);
      if (cplusplus && bp[1] == '/') {
	*comments = 1;
	bp += 2;
	while (bp < limit && *bp++ != TARGET_NEWLINE) ;
	++*newlines;
	break;
      }
      if (bp[1] != '*' || bp + 1 >= limit)
	break;
      *comments = 1;
      ...
      while (bp + 1 < limit) {
	if (bp[0] == '*'
	    && bp[1] == '\\' && bp[2] == TARGET_NEWLINE)
	  newline_fix (bp + 1);
	if (bp[0] == '*' && bp[1] == '/')
	  break;
	if (*bp == TARGET_NEWLINE) ++*newlines;
	bp++;
      }
      bp += 1;
      break;
     ...
     int newlines;
{
  ...
  while (ibp < limit) {
    *obp++ = c = *ibp++;
	...
	obp--;
	ibp++;
	while (ibp < limit && *ibp++ != TARGET_NEWLINE) ;
	break;
      }
     ...
     char *name;
{
  ...
  if (errno < sys_nerr)
    fprintf (stderr, "%s: %s\015", name, sys_errlist[errno]);
  else
    fprintf (stderr, "%s: undocumented I/O error\015", name);
